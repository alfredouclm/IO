...
# Objetivo:  funciones para calcular las medidas de desempe?o en colas M/M
...
#            - Estefan?a Jim?nez Arroyo
#            - Samuel L?pez Hern?ndez
#            - Luc?a L?pez-Rey S?nchez-Cano
#            * Permitieron su uso en cursos posteriores.
# Modif:     Alfredo Rodr?guez S?nchez
#            - correcci?n de errores
#            - reestructuraci?n del c?digo
###########
...
MM1N_medidas <- function(lambda,mu,N) {
  ...
  # Probabilidad de que est? el sistema lleno (y entonces se pierden clientes)
  pN <- MM1N_pn(lambda,mu,N,N)
  
  # Con las f?rmulas de Little
...
MMcN_medidas <- function(lambda,mu,c,N) {
  ...
  if(c==ro){
    print(paste("fi es igual a ro, no se puede usar la formula de Lq"))
    return(c(-99,-99,-99,-99,-99))
  }
